...
def compute_prevent_ascvd_10yr(data: PatientData) -> float:
    # 1. تحويل القياسات إلى نظام المتغيرات المتمركزة (Centered Variables)
    ...
    hdl_c_main = (hdl_mmol - 1.3) / 0.3    # يستخدم للقسم الأساسي
    hdl_c_inter = (hdl_mmol - 1.3) / 1.0   # يستخدم للتقاطع مع العمر (حسب الجدول)
    
    # نظام Splines للضغط
    sbp_low = (min(data.sbp, 110.0) - 110.0) / 20.0
    sbp_high = (max(data.sbp, 110.0) - 130.0) / 20.0
    
    # نظام Splines لوظائف الكلى
    ...
    # 2. حساب المجموع الجبري للوغاريتم الأرجحية (Log-Odds) - المعاملات الدقيقة لـ ASCVD
    if data.sex.lower() == "female":
        log_odds = -3.819975  # Constant
        log_odds += 0.719883 * age_c
        log_odds += 0.1176967 * non_hdl_c
        log_odds += -0.151185 * hdl_c_main
        ...
        log_odds += 0.0325692 * (age_c * hdl_c_inter)  # لاحظ استخدام hdl_c_inter
        ...
        # للذكور
        log_odds = -3.500655  # Constant
        log_odds += 0.7099847 * age_c
        log_odds += 0.1658663 * non_hdl_c
        log_odds += -0.1144285 * hdl_c_main
        ...
        log_odds += 0.0232747 * (age_c * hdl_c_inter) # لاحظ استخدام hdl_c_inter
    ...
    # 3. التحويل النهائي لنسبة مئوية (Sigmoid Probability)
